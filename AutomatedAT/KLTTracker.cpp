...
void CKLTTracker::ReadConfig()
{
	...
	// Read if outlier removal by optical flow will be performed.
	GetLine(fileInput, buffer, BUFFER_SIZE);
	sscanf(buffer, "%d", &OPT_REMOVAL);
	
	// Read ratio of std for optical flow removal 
	GetLine(fileInput, buffer, BUFFER_SIZE);
	sscanf(buffer, "%lf", &OPT_RATIO);
...
void CKLTTracker::InitializeConstants()
{
	...
	FAST_MIN_DISTANCE = MIN_DISTANCE*MIN_DISTANCE;
									...
									double rot_tx, double rot_ty)
{
	...
	for (int i = 0; i < corner_count; i++)
	{
		...
		cornersB[i].x = rot_a*x + rot_b*y + rot_tx;
		cornersB[i].y = -rot_b*x + rot_a*y + rot_ty;
	}
									...
									/*in,out*/int * cornersA_GPID)
{
	...
	for(i=0; i<corner_count; i++ ) 
	{
		double dX = cornersA[i].x - cornersB[i].x;
		double dY = cornersA[i].y - cornersB[i].y;
		dDummy = dX*dX + dY*dY;
												...
												CvPoint2D32f * coords2)
{
	...
	float a = cvmGet(x, 0, 0);
	float b = cvmGet(x, 1, 0);

	// Compute scale
	Sim2DParam.scale = sqrt(a*a + b*b);

	// Compute rotation angle
	Sim2DParam.angle = rad2deg((float)(atan2(b,a)));

	// Translation vector
	Sim2DParam.tx = cvmGet(x, 2, 0);
	Sim2DParam.ty = cvmGet(x, 3, 0);
									   ...
									   double ZA2)
{
	...
	for (i = 0; i < corner_count; i++)
	{
		...
		outCoords[i].x = dgroundX;
		outCoords[i].y = dgroundY;
	}
...
/*
		...
		if (CAL_INITIAL_GUESS == 1)
		{
		    ...
		    int nOutPt = RemoveOutOfScopePoints(cornersA, cornersB, cornersA_GPID, corner_count);
		...
		{ // Translation model tracking on rotated image
			...
			if DBL_GT(fabs(sim2Dparam.angle), MIN_ROTATED_ANGLE)
			{
				...
				cvWarpAffine(imgB, rotated_img, rot_matrix);
					...
					// DEBUG
					/*
					char rotated_name[80] = {0};
					sprintf(rotated_name, "rotated_%s", m_deqIMG[IMG_B].c_str());
					cvSaveImage(rotated_name, rotated_img);
					*/
					// DEBUG
		...
		if (CCC_CHECK == 1)
		{
			...
			for(i=0; i<corner_count; i++ ) 
			{
					...
					DBL_GT(cornersB[i].x, m_scaledIS.iWidth) || DBL_GT(cornersB[i].y, m_scaledIS.iHeight) ) 
				{
						...
						if (features_found[i]==0)
							printf("\tFeatures[%d,GP%d]:(%lf,%lf) in 1st image _not found_ with error %f\n", 
									i, cornersA_GPID[i], cornersA[i].x, cornersA[i].y, feature_errors[i]);
		...
		if (EPIPOLAR_CHECK == 1)
		{
			...
			for(i=0; i<corner_count; i++ ) 
			{
					...
					DBL_GT(cornersB[i].x, m_scaledIS.iWidth) || DBL_GT(cornersB[i].y, m_scaledIS.iHeight) ) 
				{
						...
						if (feature_errors[i] > FEATURE_ERROR)
							printf("\tFeatures[%d,GP%d]:(%lf,%lf) in 1st image found _with error %f_\n", 
									i, cornersA_GPID[i], cornersA[i].x, cornersA[i].y, feature_errors[i]);
