...
double Matrix::EuclideanNorm() const
{
	...
	for(register int r=0; r<row; r++)
		for(register int c=0; c<col; c++)
			sum += Get(r,c) * Get(r,c);
...
*/
{
	// ADHOC fix for access violation problem happening when the passed-in value are undefined.
	bool bHit;
	...
	{ 
		...
		// ADHOC fix
		if (false == bHit) 
			return false;	// since this causes access violation
